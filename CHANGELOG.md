...
## epub-wlx 0.1.0 — 2026-08-09

### Added
- **epub-wlx:** new WLX **lister plugin that reads EPUB e-books** in Double
  Commander's F3 viewer — the first EPUB plugin for DC on macOS (every existing
  one is a Windows PE `.wlx` that cannot load into a Mach-O host). Closes #19.
...
- **The book is served in-process, straight out of the ZIP**, through a
  `WKURLSchemeHandler` on a private `x-epub://` scheme — nothing is unpacked to a
...
- **Untrusted by construction:** every chapter passes through DOMPurify, only
  `x-epub:`/`data:` URIs survive rewriting (an `http(s)` reference is dropped, not
...
- Harnesses drive the real built artifact: `test/zip_test.c` (headless, in CI) and
  `test/test_host.m` (full WLX ABI, rendering, TOC, encodings, and the sanitising
  assertions) against sample books generated by `test/make_sample_epub.py`, so no
