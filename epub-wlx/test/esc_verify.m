 ...
 *
 * This is a regression net, not proof: a mock host can pass while the real app
 * fails, which is exactly what happened the first time this was fixed for
 * markdown-wlx. Always confirm Esc in Double Commander itself as well. */
...
- (void)keyDown:(NSEvent *)e {
    if (e.keyCode == 53) {
        gDCGotEsc = YES;
        NSLog(@"VERIFY: DC parent received Esc via event dispatch -> viewer closes");
    }
}
@end
...
int main(int argc, char **argv){ @autoreleasepool {
    ...
    FakeDC *dc = [[FakeDC alloc] initWithFrame:NSMakeRect(0,0,900,700)];
    win.contentView = dc;
    ...
    // Let the book settle, focus the web view, then queue Esc for normal dispatch.
